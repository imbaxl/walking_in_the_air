...
1. 解析条件：限制条件实际是一个隐函数方程组，$$\begin{cases} G(x,y,z)=0 \\ H(x,y,z)=0 \end{cases}$$ 这意味着我可以得到 Jacobi 矩阵 $J=\begin{pmatrix} G_{x} & G_{y} & G_{z} \\ H_{x} & H_{y} & H_{z} \end{pmatrix}$ ，再假设 $\text{r}(J)=2$ 保证 [[隐函数#^d51d0b|隐函数定理4]] 在点 $(x_{0},y_{0},z_{0})$ 可以得到显式的 $y=y(x),z=z(x)$
